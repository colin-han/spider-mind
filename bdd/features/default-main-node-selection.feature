Feature: 思维导图主节点默认选中功能
  ...
  Scenario: 页面刷新后根节点保持选中状态
    Given 我已经创建了一个仅包含主节点的思维导图，名字叫："页面刷新后根节点保持选中状态"
    When 我点击"页面刷新后根节点保持选中状态"思维导图
    Then 我应该进入思维导图编辑页面
    And 节点"root"应该被选中
    When 我刷新页面
    Then 页面应该保持在当前思维导图中
  ...
  Scenario: 根节点选中状态下可正常进行编辑操作
    Given 我已经创建了一个仅包含主节点的思维导图，名字叫："根节点选中状态下可正常进行编辑操作"
    When 我点击"根节点选中状态下可正常进行编辑操作"思维导图
    Then 我应该进入思维导图编辑页面
    And 节点"root"应该被选中
    When 我双击节点"root"
  ...
  Scenario: 根节点选中状态下可正常添加子节点
    Given 我已经创建了一个仅包含主节点的思维导图，名字叫："根节点选中状态下可正常添加子节点"
    When 我点击"根节点选中状态下可正常添加子节点"思维导图
    Then 我应该进入思维导图编辑页面
    And 节点"root"应该被选中
  ...
  Scenario: 选择其他节点后再选择根节点仍显示正确状态
    Given 我已经创建了一个仅包含主节点的思维导图，名字叫："选择其他节点后再选择根节点仍显示正确状态"
    When 我点击"选择其他节点后再选择根节点仍显示正确状态"思维导图
    Then 我应该进入思维导图编辑页面
    And 我为节点"root"添加子节点
    When 我选中节点"root-0"
