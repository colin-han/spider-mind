Feature: 思维导图主节点默认选中功能
  ...
  Scenario: 打开现有思维导图时根节点默认选中
    Given 用户已经创建了一个仅包含主节点的思维导图，名字叫："打开现有思维导图时根节点默认选中"
    When 用户点击"打开现有思维导图时根节点默认选中"思维导图
    Then 用户应该进入思维导图编辑页面
    And 节点"root"应该存在
    And 节点"root"应该被选中

  Scenario: 页面刷新后根节点保持选中状态
    Given 用户已经创建了一个仅包含主节点的思维导图，名字叫："页面刷新后根节点保持选中状态"
    When 用户点击"页面刷新后根节点保持选中状态"思维导图
    Then 用户应该进入思维导图编辑页面
    And 节点"root"应该被选中
    When 用户刷新页面
    Then 页面应该保持在当前思维导图中
  ...
  Scenario: 根节点选中状态下可正常进行编辑操作
    Given 用户已经创建了一个仅包含主节点的思维导图，名字叫："根节点选中状态下可正常进行编辑操作"
    When 用户点击"根节点选中状态下可正常进行编辑操作"思维导图
    Then 用户应该进入思维导图编辑页面
    And 节点"root"应该被选中
    When 用户双击节点"root"
    ...
    When 用户输入"测试根节点内容"
    And 用户按下"Enter"键
    Then 节点"root"应该退出编辑状态
    And 节点"root"的内容应该是"测试根节点内容"
    And 节点"root"应该被选中

  Scenario: 根节点选中状态下可正常添加子节点
    Given 用户已经创建了一个仅包含主节点的思维导图，名字叫："根节点选中状态下可正常添加子节点"
    When 用户点击"根节点选中状态下可正常添加子节点"思维导图
    Then 用户应该进入思维导图编辑页面
    And 节点"root"应该被选中
    When 用户为节点"root"添加子节点
    Then 节点"root"应该有1个子节点
    And 节点"root-0"应该存在
    And 节点"root-0"应该是节点"root"的子节点

  Scenario: 选择其他节点后再选择根节点仍显示正确状态
    Given 用户已经创建了一个仅包含主节点的思维导图，名字叫："选择其他节点后再选择根节点仍显示正确状态"
    When 用户点击"选择其他节点后再选择根节点仍显示正确状态"思维导图
    Then 用户应该进入思维导图编辑页面
    And 节点"root"应该被选中
    And 用户为节点"root"添加子节点
    When 用户选中节点"root-0"
