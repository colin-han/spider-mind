...
Feature: Test-ID生成规则稳定性验证
  ...
  Scenario: root节点的兄弟节点应该是float节点
    Given 用户点击"新建思维导图"按钮
    When 用户按下"Enter"键
    Then 节点"float-0"应该存在
    And 用户修改节点"float-0"的内容为"浮动节点1"
    When 用户按下"Enter"键
    Then 节点"float-1"应该存在
    And 用户修改节点"float-1"的内容为"浮动节点2"
    When 用户按下"Enter"键
    Then 节点"float-2"应该存在
    And 用户修改节点"float-2"的内容为"浮动节点3"
    When 用户删除节点"float-0"
    # And 等待10秒
