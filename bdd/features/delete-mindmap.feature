Feature: 删除思维导图功能
  ...
  Scenario: 成功删除思维导图
    Given 用户已经创建了一个仅包含主节点的思维导图，名字叫："项目规划"
    When 用户点击"项目规划"思维导图卡片上的删除按钮
    Then 系统显示删除确认对话框
    And 对话框标题为"删除思维导图"
    And 对话框内容包含"确定要删除思维导图 \"项目规划\" 吗？此操作不可撤销。"
    When 用户点击确认对话框中的"确认"按钮
  ...
  Scenario: 取消删除操作
    Given 用户已经创建了一个仅包含主节点的思维导图，名字叫："学习计划"
    When 用户点击"学习计划"思维导图卡片上的删除按钮
    Then 系统显示删除确认对话框
    When 用户点击确认对话框中的"取消"按钮
    Then 删除确认对话框关闭
  ...
  Scenario: 点击对话框外部区域不会取消删除
    Given 用户已经创建了一个仅包含主节点的思维导图，名字叫："工作笔记"
    When 用户点击"工作笔记"思维导图卡片上的删除按钮
    Then 系统显示删除确认对话框
    When 用户点击对话框外部区域
    Then 删除确认对话框仍然存在
  ...
  Scenario: 使用ESC键取消删除操作
    Given 用户已经创建了一个仅包含主节点的思维导图，名字叫："读书笔记"
    When 用户点击"读书笔记"思维导图卡片上的删除按钮
    Then 系统显示删除确认对话框
    When 用户按下"ESC"键
    Then 删除确认对话框关闭
  ...
  Scenario: 删除不存在的思维导图
    Given 用户已经创建了一个仅包含主节点的思维导图，名字叫："临时笔记"
    And 该思维导图已被其他方式删除
    When 用户点击"临时笔记"思维导图卡片上的删除按钮
    And 用户点击确认对话框中的"确认"按钮
    Then 系统显示错误提示"删除失败: 思维导图不存在"
    And 思维导图列表刷新移除不存在的条目

  Scenario: 防止删除按钮误触
    Given 用户已经创建了一个仅包含主节点的思维导图，名字叫："重要文档"
    When 用户鼠标悬停在思维导图卡片上
    Then 删除按钮从不可见变为可见
    When 用户鼠标移出思维导图卡片
    Then 删除按钮恢复为不可见状态
    When 用户点击思维导图卡片内容区域（非删除按钮）
    Then 用户导航到思维导图编辑页面
    And 没有触发删除操作
